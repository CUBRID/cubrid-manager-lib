 ...
 *******************************************************************************/
package com.cubrid.common.ui.query.control.jface.text.contentassist;
...
import com.cubrid.common.ui.query.control.jface.contentassist.IContentAssistSubjectControl;
 ...
 * @version CM/CQB 8.4.1 - 2012-04 modify by fulei 
 * add left and right button listener
 */
class CompletionProposalPopup implements IContentAssistListener {
	...
	private boolean isShowSecondProposal = false;
	 ...
	 * @return an error message or <code>null</code> in case of no error
	 */
	public String showSecondProposals(final boolean autoActivated, ICompletionProposal selectedProposal) {
		
		ICompletionProposal[] proposal = computeSecondProposals(fInvocationOffset, selectedProposal);
		
		if (proposal.length ==0) {
			return "";
		}
		isShowSecondProposal = true;
		fComputedProposals= proposal;
		setProposals(fComputedProposals, false);
		displayProposals();
		return getErrorMessage();
	}
	...
	/**
	 * show first proposals from second proposals
	 *
	 * @param autoActivated <code>true</code> if auto activation context
	 * @return an error message or <code>null</code> in case of no error
	 */
	public String backToFirstProposals(final boolean autoActivated) {
		ICompletionProposal[] proposal = computeProposals(fInvocationOffset);
		if (proposal.length ==0) {
			return "";
		}
		fComputedProposals= proposal;
	 ...
	 * @return the completion proposals available at this offset
	 */
	private ICompletionProposal[] computeSecondProposals(int offset, ICompletionProposal selectedProposal) {
		if (fContentAssistSubjectControl != null)
			return fContentAssistant.computeSecondProposals(fContentAssistSubjectControl, offset, selectedProposal);
		return fContentAssistant.computeSecondProposals(fViewer, offset, selectedProposal);
	}
	...
	public boolean verifyKey(VerifyEvent e) {
		...
		if (key == 0) {
			...
			switch (e.keyCode) {
				...
				case SWT.ARROW_LEFT :
					if (isShowSecondProposal) {
						backToFirstProposals(false);
						isShowSecondProposal = false;
						break;
					} else {
						filterProposals();
						return true;
				...
				case SWT.ARROW_RIGHT :
					String result = showSecondProposals(false, getSelectedProposal());
